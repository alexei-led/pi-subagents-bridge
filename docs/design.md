...
- Native lifecycle comes from `pi-subagents` status. A process-terminal proof is either the validated native `details.lifecycleStatus.processTerminal`, a cached `subagent:process-terminal` event, or a workflow proof synthesized from a closed `workflowChildren` inventory plus each child's attested writer-exit proof. It is never inferred from PID, prompt, or elapsed time.
