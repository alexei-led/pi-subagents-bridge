...
- Native lifecycle comes from targeted `pi-subagents` status. Observed process-terminal proofs require a matching runner instance; pending and unknown status proofs are diagnostic only. Cached `subagent:process-terminal` events are validated observed proofs, and a validated native status proof takes precedence over the cache. A workflow proof is passed through only from native `details.workflowTerminalProof` when it is observed, dispatch is closed, and each child is observed or not-started. Missing, malformed, pending, or unknown workflow proofs fail closed; `workflowChildren` and child process-terminal files never synthesize one. Terminal evidence is never inferred from status text, PID, prompt, or elapsed time.
