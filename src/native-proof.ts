...
export function attestUpstreamTerminalProof(
  ...
  if (!record(proof) || proof.version !== 1 || proof.state !== "observed" || proof.runId !== runId) return undefined;
  if (typeof proof.observedAt !== "number" || !Number.isFinite(proof.observedAt)) return undefined;
  if (!text(proof.runnerProcessInstanceId)) return undefined;
  if (!Array.isArray(proof.instances) && !record(proof.writers)) return undefined;
  return proof;
}
